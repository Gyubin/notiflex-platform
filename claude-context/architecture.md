...
> **기준 시점: 8장 완료 (2026-07-27)** · 이미지 `v0.4.0`
> 이 문서는 "지금 어떻게 동작하는가"만 담는다. 구조가 바뀌면 이 파일을 갱신한다.

## 세 문서의 역할

이 저장소는 AI가 참조하는 문서를 세 층으로 나눈다. 섞이면 어느 것도 믿을 수 없게 된다.
...
`AGENTS.md`는 "이렇게 해라", 이 문서는 "지금 이렇게 되어 있다", ADR은 "그때 왜 그렇게 정했나"다.
결정의 **이유**는 ADR에만 쓰고 여기 옮기지 않는다. 진행 이력과 트러블슈팅은 `JOURNEY.md`에 있다.
